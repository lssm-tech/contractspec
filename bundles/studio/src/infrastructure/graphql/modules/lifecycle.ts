...
export function registerLifecycleSchema(
  builder: typeof contractSpecStudioSchemaBuilder,
  options?: { service?: LifecycleAssessmentService }
) {
  if (debugGraphQL) {
  ...
  builder.queryFields((t) => ({
    ...
    lifecycleRecommendations: t.field({
      ...
      resolve: async (_root, _args, ctx) => {
        const user = requireAuthAndGet(ctx);
        const profile = await getOrCreateProfile(user.organizationId);
        const service = options?.service ?? lifecycleService;
        return service.getStagePlaybook(
