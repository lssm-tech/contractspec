...
import type {
  IntegrationCategory,
  IntegrationSpec,
} from '@contractspec/lib.contracts/integrations/spec';
...
} from '@contractspec/lib.contracts/integrations/secrets';
import type {
  SecretPayloadEncoding,
  SecretProvider,
} from '@contractspec/lib.contracts/integrations/secrets/provider';
...
export class PlatformIntegrationsModule {
  ...
  async createConnection(
    ...
    const spec = registry.get(
      ...
      String(input.integrationVersion)
    );
    if (!spec) {
      throw new Error(
    ...
    const record = await prisma.integrationConnection.create({
      data: {
        ...
        integrationVersion: parseInt(spec.meta.version, 10),
        label: input.label,
