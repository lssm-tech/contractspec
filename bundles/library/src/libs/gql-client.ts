...
const authClient = {
  useSession: () => ({ data: { session: { token: '', userId: '' } } }),
};
export {
  graphql,
  type TypedDocumentString,
...
export const useGraphQL = <TDocument extends DocumentTypeDecoration<any, any>>(
  ...
  return useQuery({
    ...
    queryFn: async ({ queryKey }) => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Cast to bypass complex generic inference
      const vars = queryKey[1] as any;
      return gqlRequest(
        document,
        vars,
        sessionData?.session?.token
