...
const curatedExamples: ExampleCheck[] = [
	{
		...
		steps: [
			{
				label: 'build',
				command: [tscExecutable, '--project', 'tsconfig.json'],
			},
			{ label: 'typecheck', command: [tscExecutable, '--noEmit'] },
			{ label: 'test', command: [bunExecutable, 'test'] },
			{
				label: 'smoke',
				command: [bunExecutable, 'test', 'src/example.smoke.test.ts'],
			},
		],
	},
	{
	...
	},
	{
		...
		steps: [
			...
			{
				label: 'build:bundle',
				command: [contractspecBuildExecutable, 'transpile'],
			},
...
async function checkUrls(): Promise<void> {
	...
	for (const url of urls) {
		...
		try {
			...
			if (!response.ok) {
				fail(
					`Reachability probe failed for ${url} with status ${response.status}`
				);
			}
		} catch (error) {
