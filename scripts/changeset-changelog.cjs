...
function readCapsule(changesetId, cwd) {
	const capsulePath = path.join(cwd, ".changeset", `${changesetId}.release.yaml`);
...
async function getReleaseLine(changeset, type) {
	...
	const summary = capsule && typeof capsule.summary === "string"
		? capsule.summary
		: changeset.summary;
	const lines = [`- ${summary}`];

	if (capsule && Array.isArray(capsule.packages) && capsule.packages.length > 0) {
		lines.push(
			`  - Packages: ${capsule.packages
				.map((pkg) => `${pkg.name}${pkg.version ? `@${pkg.version}` : ""} (${pkg.releaseType || type})`)
				.join(", ")}`
		);
	}

	if (capsule && Array.isArray(capsule.migrationInstructions) && capsule.migrationInstructions.length > 0) {
		lines.push(
	...
	if (capsule && Array.isArray(capsule.deprecations) && capsule.deprecations.length > 0) {
