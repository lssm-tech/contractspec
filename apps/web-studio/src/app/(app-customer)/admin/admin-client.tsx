...
import { useAuthContext } from '@contractspec/bundle.studio/presentation/providers/auth';
import { authClient } from '@contractspec/bundle.studio/presentation/providers/auth/client';
import {
  gqlRequest,
  graphql,
} from '@contractspec/bundle.studio/presentation/libs/gql-client';
import {
...
interface PlatformIntegrationSpec {
  key: string;
  version: number;
  category: string;
  title: string;
...
export default function StudioAdminClient() {
  ...
  const orgsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.platformAdminOrganizations as unknown as PlatformOrg[];
    },
    staleTime: 30_000,
  });
  ...
  const specsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      return result.platformAdminIntegrationSpecs as PlatformIntegrationSpec[];
    },
    staleTime: 60_000,
  });
    ...
    queryFn: async () => {
      ...
      const result = await gqlRequest(
        CONNECTIONS_QUERY,
        {
          ...
          } as unknown as PlatformIntegrationConnectionListInput,
        },
        token ? { Authorization: `Bearer ${token}` } : {}
      );
      return result.platformAdminIntegrationConnections as unknown as PlatformIntegrationConnection[];
    },
    staleTime: 15_000,
  });
  ...
  const specOptions = React.useMemo(() => {
    return specs
      ...
      .map((s) => ({
        id: `${s.key}::${s.version}`,
        label: `${s.title} · ${s.key}.v${s.version}`,
        spec: s,
      }));
  }, [specs]);
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const result = await gqlRequest(
        ...
        {
          input:
            variables.input as unknown as PlatformIntegrationConnectionCreateInput,
        },
        token ? { Authorization: `Bearer ${token}` } : {}
      );

      return result.platformAdminIntegrationConnectionCreate as unknown as PlatformIntegrationConnection;
    },
    onSuccess: async () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async () => {
      ...
      const result = await gqlRequest(
        ...
        {
          input:
            variables.input as unknown as PlatformIntegrationConnectionUpdateInput,
        },
        token ? { Authorization: `Bearer ${token}` } : {}
      );

      return result.platformAdminIntegrationConnectionUpdate as unknown as PlatformIntegrationConnection;
    },
    onSuccess: async () => {
      await connectionsQuery.refetch();
      setEditingConnectionId(null);
  ...
  const deleteMutation = useMutation({
    mutationFn: async (connectionId: string) => {
      ...
      return result.platformAdminIntegrationConnectionDelete;
    },
    onSuccess: async () => {
      await connectionsQuery.refetch();
    },
