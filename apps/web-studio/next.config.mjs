...
const nextConfig = {
    ...
    return [
      {
        source: '/api/chat',
        destination:
          process.env.API_STUDIO_URL || 'https://api.contractspec.studio/api/chat',
      },
      // {
      //   source: '/api/graphql',
      ...
      {
        source: '/:path*',
        has: [{ type: 'host', value: 'llms.contractspec.studio' }],
        destination: '/llms.txt',
      },
      {
  ...
  async redirects() {
    ...
    const apiLlmsUrl =
      process.env.API_CONTRACTSPEC_URL || `https://llms.contractspec.studio`;
    return [
