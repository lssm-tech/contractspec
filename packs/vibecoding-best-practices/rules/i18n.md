...
The repository now uses Biome as the only formatter and linter entrypoint. `biome.jsonc` owns the repo-wide formatting baseline, native lint rules, and import organization.

- **Biome** (`biome.jsonc`): Runs through `bun run lint:check` and `bun run lint:fix`.
- **ContractSpec policy checks** (`contractspec ci --checks policy`): Enforce contract-first architectural guardrails that do not belong in the formatter.

Literal user-facing copy still follows this document's conventions. Until a dedicated i18n-specific policy check lands, validate translation completeness with `bun run i18n:check` and keep JSX copy routed through the established `t()` or typed copy-file patterns.
...
- **Lint**: `bun run lint:check` runs the Biome pipeline.
