...
- Use the approved PostHog SDK; keys/secrets come from env vars only.
- Ensure capture endpoints and api/host URLs are set per environment; default to production-safe sampling.
...
- Verify event name, props, distinct_id/user/tenant IDs are set.
- Ensure no PII/secrets in props; hash or drop sensitive data.
- Confirm flags/properties enums live in a single source of truth.
- Add sampling when volume is high; document sampling rate per environment.
