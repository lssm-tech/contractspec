...
export const LessonTypeEnum = defineEntityEnum({
  name: 'LessonType',
  values: ['CONTENT', 'VIDEO', 'INTERACTIVE', 'QUIZ', 'PRACTICE', 'PROJECT'] as const,
  schema: 'lssm_learning',
...
export const CourseEntity = defineEntity({
  ...
  fields: {
    ...
    description: field.string({ isOptional: true, description: 'Course description' }),
    summary: field.string({ isOptional: true, description: 'Short summary' }),
    
    // Classification
    difficulty: field.enum('CourseDifficulty', { default: 'BEGINNER', description: 'Difficulty level' }),
    category: field.string({ isOptional: true, description: 'Course category' }),
    tags: field.json({ isOptional: true, description: 'Tags for discovery' }),
    
    // Prerequisites
    prerequisites: field.json({ isOptional: true, description: 'Required course IDs' }),
    requiredSkills: field.json({ isOptional: true, description: 'Required skill levels' }),
    
    // Duration
    estimatedDuration: field.int({ isOptional: true, description: 'Estimated duration in minutes' }),
    
    // Media
    thumbnailUrl: field.string({ isOptional: true, description: 'Thumbnail image URL' }),
    coverImageUrl: field.string({ isOptional: true, description: 'Cover image URL' }),
    promoVideoUrl: field.string({ isOptional: true, description: 'Promo video URL' }),
    
    // Publishing
    status: field.enum('CourseStatus', { default: 'DRAFT', description: 'Publication status' }),
    publishedAt: field.dateTime({ isOptional: true, description: 'When published' }),
    ...
    // Settings
    isPublic: field.boolean({ default: false, description: 'Whether course is publicly accessible' }),
    isFeatured: field.boolean({ default: false, description: 'Whether course is featured' }),
    certificateEnabled: field.boolean({ default: false, description: 'Award certificate on completion' }),
...
export const CourseModuleEntity = defineEntity({
  ...
  fields: {
    ...
    description: field.string({ isOptional: true, description: 'Module description' }),
    
    // Ordering
    order: field.int({ default: 0, description: 'Display order' }),
    
    // Unlock conditions
    unlockCondition: field.json({ isOptional: true, description: 'Conditions to unlock module' }),
    prerequisiteModuleIds: field.json({ isOptional: true, description: 'Required modules to complete first' }),
...
export const LessonEntity = defineEntity({
  ...
  fields: {
    ...
    description: field.string({ isOptional: true, description: 'Lesson description' }),
    
    // Type and content
    type: field.enum('LessonType', { default: 'CONTENT', description: 'Lesson type' }),
    ...
    // XP reward
    xpReward: field.int({ default: 10, description: 'XP awarded on completion' }),
    
    // Settings
    isFree: field.boolean({ default: false, description: 'Whether lesson is free preview' }),
    isRequired: field.boolean({ default: true, description: 'Whether lesson is required for completion' }),
...
export const LessonContentEntity = defineEntity({
  ...
  fields: {
    ...
    content: field.string({ isOptional: true, description: 'Text content (markdown, etc.)' }),
    mediaUrl: field.string({ isOptional: true, description: 'Media URL for video/audio' }),
    embedData: field.json({ isOptional: true, description: 'Embed data for external content' }),
    
    // Ordering
    order: field.int({ default: 0, description: 'Display order' }),
    
    // Duration
    duration: field.int({ isOptional: true, description: 'Content duration in seconds' }),
