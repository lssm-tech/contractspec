...
export const EnrollmentStatusEnum = defineEntityEnum({
  ...
  values: ['ENROLLED', 'IN_PROGRESS', 'COMPLETED', 'DROPPED', 'EXPIRED'] as const,
...
export const LearnerEntity = defineEntity({
  ...
  fields: {
    ...
    // Profile
    displayName: field.string({ isOptional: true, description: 'Display name' }),
    ...
    // Settings
    timezone: field.string({ default: '"UTC"', description: 'Learner timezone' }),
    dailyGoalXp: field.int({ default: 50, description: 'Daily XP goal' }),
    reminderEnabled: field.boolean({ default: true, description: 'Enable reminders' }),
    reminderTime: field.string({ isOptional: true, description: 'Preferred reminder time' }),
...
export const EnrollmentEntity = defineEntity({
  ...
  fields: {
    ...
    // Status
    status: field.enum('EnrollmentStatus', { default: 'ENROLLED', description: 'Enrollment status' }),
    
    // Progress
    progress: field.int({ default: 0, description: 'Completion percentage (0-100)' }),
    completedLessons: field.int({ default: 0, description: 'Number of completed lessons' }),
    totalLessons: field.int({ default: 0, description: 'Total lessons in course' }),
    
    // XP
    xpEarned: field.int({ default: 0, description: 'XP earned in this course' }),
    
    // Timeline
    startedAt: field.dateTime({ isOptional: true, description: 'When learner started' }),
    completedAt: field.dateTime({ isOptional: true, description: 'When learner completed' }),
    lastAccessedAt: field.dateTime({ isOptional: true, description: 'Last access time' }),
    
    // Certificate
    certificateId: field.string({ isOptional: true, description: 'Issued certificate ID' }),
...
export const LessonProgressEntity = defineEntity({
  ...
  fields: {
    ...
    // Status
    status: field.enum('ProgressStatus', { default: 'NOT_STARTED', description: 'Progress status' }),
    
    // Progress details
    progress: field.int({ default: 0, description: 'Completion percentage (0-100)' }),
    score: field.int({ isOptional: true, description: 'Score achieved (for quizzes)' }),
    
    // Attempts
    attempts: field.int({ default: 0, description: 'Number of attempts' }),
    bestScore: field.int({ isOptional: true, description: 'Best score across attempts' }),
    
    // Time tracking
    timeSpent: field.int({ default: 0, description: 'Time spent in seconds' }),
    
    // XP
    xpEarned: field.int({ default: 0, description: 'XP earned from this lesson' }),
    ...
    // Bookmarks and notes
    bookmarks: field.json({ isOptional: true, description: 'Content bookmarks' }),
...
export const ModuleCompletionEntity = defineEntity({
  ...
  indexes: [
    index.unique(['learnerId', 'moduleId'], { name: 'module_completion_unique' }),
...
export const CertificateEntity = defineEntity({
  ...
  fields: {
    ...
    // Certificate details
    certificateNumber: field.string({ isUnique: true, description: 'Unique certificate number' }),
    title: field.string({ description: 'Certificate title' }),
    description: field.string({ isOptional: true, description: 'Certificate description' }),
    ...
    validUntil: field.dateTime({ isOptional: true, description: 'Expiration date' }),
    
    // Verification
    verificationUrl: field.string({ isOptional: true, description: 'Verification URL' }),
    credentialHash: field.string({ isOptional: true, description: 'Credential hash for verification' }),
    
    // Status
    isRevoked: field.boolean({ default: false, description: 'Whether certificate is revoked' }),
    revokedAt: field.dateTime({ isOptional: true, description: 'When revoked' }),
    revokedReason: field.string({ isOptional: true, description: 'Revocation reason' }),
