...
export const DeckEntity = defineEntity({
  ...
  fields: {
    ...
    description: field.string({ isOptional: true, description: 'Deck description' }),
    ...
    // Visibility
    isPublic: field.boolean({ default: false, description: 'Whether deck is publicly visible' }),
    ...
    // Settings
    newCardsPerDay: field.int({ default: 20, description: 'New cards to introduce per day' }),
    reviewsPerDay: field.int({ default: 100, description: 'Maximum reviews per day' }),
...
export const CardEntity = defineEntity({
  ...
  fields: {
    ...
    explanation: field.string({ isOptional: true, description: 'Detailed explanation' }),
    
    // Media
    frontMediaUrl: field.string({ isOptional: true, description: 'Media for front' }),
    backMediaUrl: field.string({ isOptional: true, description: 'Media for back' }),
    audioUrl: field.string({ isOptional: true, description: 'Audio pronunciation' }),
    ...
    // Status
    isSuspended: field.boolean({ default: false, description: 'Whether card is suspended' }),
  ...
  indexes: [
    index.on(['deckId', 'order']),
    index.on(['isSuspended']),
  ],
});
...
export const CardReviewEntity = defineEntity({
  ...
  fields: {
    ...
    // Time tracking
    responseTimeMs: field.int({ isOptional: true, description: 'Time to respond in ms' }),
    ...
    // Review type
    reviewType: field.string({ default: '"review"', description: 'Type: new, learning, review, relearning' }),
...
export const CardScheduleEntity = defineEntity({
  ...
  fields: {
    ...
    // SRS state
    interval: field.int({ default: 0, description: 'Current interval in days' }),
    easeFactor: field.decimal({ default: 2.5, description: 'Ease factor (SM-2)' }),
    repetitions: field.int({ default: 0, description: 'Number of successful repetitions' }),
    
    // Scheduling
    nextReviewAt: field.dateTime({ description: 'When next review is due' }),
    lastReviewAt: field.dateTime({ isOptional: true, description: 'When last reviewed' }),
    
    // Learning state
    learningStep: field.int({ default: 0, description: 'Current learning step' }),
    isGraduated: field.boolean({ default: false, description: 'Whether card has graduated' }),
    isRelearning: field.boolean({ default: false, description: 'Whether card is being relearned' }),
    
    // Stats
    lapses: field.int({ default: 0, description: 'Number of times card was forgotten' }),
