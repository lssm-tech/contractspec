...
export function parseImportedSpecNames(
	...
	const importRegex =
		/import\s+.*?\s+from\s+['"]([^'"]+\.(?:contracts|contract|command|query|operation|operations|event|presentation|workflow|data-view|migration|telemetry|experiment|app-config|integration|knowledge)(?:\.[jt]s)?)['"]/g;
