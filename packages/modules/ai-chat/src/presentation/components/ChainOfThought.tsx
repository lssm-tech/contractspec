...
export function ChainOfThoughtHeader({
	...
	return (
		<CollapsibleTrigger
			className={cn(
				'flex w-full cursor-pointer items-center gap-2 rounded-md px-3 py-2 font-medium text-sm transition-colors hover:bg-muted',
				className
			)}
		>
			<ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground transition-transform group-data-[state=open]/cot:rotate-180" />
			{children}
		</CollapsibleTrigger>
	);
}
...
export function ChainOfThoughtStep({
	...
	return (
		<div
			className={cn(
				'flex gap-3 border-border border-b py-2 last:border-b-0',
				className
			)}
		>
			<div
...
export function ChainOfThoughtContent({
	...
	return (
		<CollapsibleContent>
			<div
				className={cn(
					'mt-1 rounded-md border border-border bg-muted px-3 py-2',
					className
				)}
			>
				{children}
