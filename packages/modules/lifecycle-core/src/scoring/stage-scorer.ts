...
type MetricWeightConfig = {
  weight: number;
  threshold: number;
  direction?: 'gte' | 'lte';
};

type StageWeightConfig = {
  base: number;
...
export class StageScorer {
  ...
  score(input: StageScoreInput): LifecycleScore[] {
    ...
    const scores = Object.values(LifecycleStage).filter(isStageValue).map((stage) => {
      ...
      let contributions = 0;
      const totalPossible =
        Object.keys(config.metrics ?? {}).length + Object.keys(config.signalKinds ?? {}).length || 1;
...
const passesThreshold = (value: number, config: MetricWeightConfig): boolean => {
...
const evaluateSignalKinds = (signals: LifecycleSignal[]): Record<string, number> =>
