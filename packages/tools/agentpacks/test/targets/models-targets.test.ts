...
import { FEATURE_IDS, WorkspaceConfigSchema } from '../../src/core/config.js';
import {
	FeatureMerger,
	type MergedFeatures,
...
describe('models feature loaded correctly', () => {
	test('models config is present in merged features', () => {
		...
		expect(mergedFeatures.models!.default).toBe('anthropic/claude-sonnet-4-6');
...
describe('OpenCodeTarget — models', () => {
	...
	test('generates per-agent model assignments in opencode.json', () => {
		...
		expect(content.agent.reviewer.model).toBe('anthropic/claude-opus-4-6');
