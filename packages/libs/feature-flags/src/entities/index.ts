...
export const RuleOperatorEnum = defineEntityEnum({
  name: 'RuleOperator',
  values: ['EQ', 'NEQ', 'IN', 'NIN', 'CONTAINS', 'NOT_CONTAINS', 'GT', 'GTE', 'LT', 'LTE', 'PERCENTAGE'] as const,
  schema: 'lssm_feature_flags',
...
export const FeatureFlagEntity = defineEntity({
  ...
  fields: {
    ...
    key: field.string({ isUnique: true, description: 'Flag key (e.g., new_dashboard)' }),
    name: field.string({ description: 'Human-readable name' }),
    description: field.string({ isOptional: true, description: 'Description of the flag' }),
    
    // Status and default value
    status: field.enum('FlagStatus', { default: 'OFF', description: 'Flag status' }),
    defaultValue: field.boolean({ default: false, description: 'Default value when no rules match' }),
    
    // Multivariate support
    variants: field.json({ isOptional: true, description: 'Variant definitions for multivariate flags' }),
    
    // Scope
    orgId: field.string({ isOptional: true, description: 'Organization scope (null = global)' }),
    
    // Metadata
    tags: field.json({ isOptional: true, description: 'Tags for categorization' }),
...
export const FlagTargetingRuleEntity = defineEntity({
  ...
  fields: {
    ...
    // Rule definition
    name: field.string({ isOptional: true, description: 'Rule name for debugging' }),
    priority: field.int({ default: 0, description: 'Rule priority (lower = higher priority)' }),
    enabled: field.boolean({ default: true, description: 'Whether rule is active' }),
    ...
    // Result
    rolloutPercentage: field.int({ isOptional: true, description: 'Percentage for gradual rollout (0-100)' }),
    serveValue: field.boolean({ isOptional: true, description: 'Boolean value to serve' }),
    serveVariant: field.string({ isOptional: true, description: 'Variant key to serve (for multivariate)' }),
...
export const ExperimentEntity = defineEntity({
  ...
  fields: {
    ...
    name: field.string({ description: 'Human-readable name' }),
    description: field.string({ isOptional: true, description: 'Experiment description' }),
    hypothesis: field.string({ isOptional: true, description: 'Experiment hypothesis' }),
    
    // Associated flag
    flagId: field.foreignKey({ description: 'Associated feature flag' }),
    
    // Configuration
    status: field.enum('ExperimentStatus', { default: 'DRAFT', description: 'Experiment status' }),
    ...
    // Targeting
    audiencePercentage: field.int({ default: 100, description: 'Percentage of audience to include' }),
    audienceFilter: field.json({ isOptional: true, description: 'Audience filter criteria' }),
    
    // Timeline
    scheduledStartAt: field.dateTime({ isOptional: true, description: 'Scheduled start time' }),
    scheduledEndAt: field.dateTime({ isOptional: true, description: 'Scheduled end time' }),
    startedAt: field.dateTime({ isOptional: true, description: 'Actual start time' }),
    endedAt: field.dateTime({ isOptional: true, description: 'Actual end time' }),
    
    // Results
    winningVariant: field.string({ isOptional: true, description: 'Declared winning variant' }),
    results: field.json({ isOptional: true, description: 'Experiment results summary' }),
...
export const ExperimentAssignmentEntity = defineEntity({
  ...
  fields: {
    ...
    // Context
    context: field.json({ isOptional: true, description: 'Context at assignment time' }),
  ...
  indexes: [
    index.unique(['experimentId', 'subjectType', 'subjectId'], { name: 'experiment_assignment_unique' }),
...
export const FlagEvaluationEntity = defineEntity({
  ...
  fields: {
    ...
    variant: field.string({ isOptional: true, description: 'Served variant (for multivariate)' }),
    
    // Match info
    matchedRuleId: field.string({ isOptional: true, description: 'Rule that matched (if any)' }),
    reason: field.string({ description: 'Evaluation reason (default, rule, experiment, etc.)' }),
    
    // Context
    context: field.json({ isOptional: true, description: 'Evaluation context' }),
