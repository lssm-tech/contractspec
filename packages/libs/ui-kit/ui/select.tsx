...
function SelectTrigger({
	...
	return (
		<SelectPrimitive.Trigger
			ref={ref}
			className={cn(
				'flex h-10 native:h-12 flex-row items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-muted-foreground text-sm web:ring-offset-background web:focus:outline-hidden web:focus:ring-2 web:focus:ring-ring web:focus:ring-offset-2 [&>span]:line-clamp-1',
			...
			>
				<Animated.View className="z-50" entering={FadeIn} exiting={FadeOut}>
					<SelectPrimitive.Content
						className={cn(
							'data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 relative z-50 max-h-96 min-w-32 rounded-md border border-border bg-popover px-1 py-2 shadow-foreground/10 shadow-md',
							position === 'popper' &&
...
function SelectItem({
	...
	return (
		<SelectPrimitive.Item
			className={cn(
				'web:group relative flex w-full web:cursor-default web:select-none flex-row items-center rounded-xs native:py-2 py-1.5 pr-2 native:pl-10 pl-8 web:outline-hidden web:hover:bg-accent/50 web:focus:bg-accent active:bg-accent',
