...
export const JobStatusEnum = defineEntityEnum({
  name: 'JobStatus',
  values: ['PENDING', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED', 'DEAD_LETTER'] as const,
  schema: 'lssm_jobs',
...
export const JobEntity = defineEntity({
  ...
  fields: {
    ...
    // Execution tracking
    attempts: field.int({ default: 0, description: 'Number of execution attempts' }),
    maxRetries: field.int({ default: 3, description: 'Maximum retry attempts' }),
    lastError: field.string({ isOptional: true, description: 'Last error message' }),
    lastErrorStack: field.string({ isOptional: true, description: 'Last error stack trace' }),
    
    // Timing
    scheduledAt: field.dateTime({ isOptional: true, description: 'When job should be processed' }),
    startedAt: field.dateTime({ isOptional: true, description: 'When processing started' }),
    completedAt: field.dateTime({ isOptional: true, description: 'When processing completed' }),
    timeoutAt: field.dateTime({ isOptional: true, description: 'Job timeout deadline' }),
    
    // Deduplication
    dedupeKey: field.string({ isOptional: true, description: 'Key for deduplication' }),
    
    // Context
    tenantId: field.string({ isOptional: true, description: 'Tenant/org context' }),
    userId: field.string({ isOptional: true, description: 'User who enqueued' }),
...
export const ScheduledJobEntity = defineEntity({
  ...
  fields: {
    ...
    timezone: field.string({ default: '"UTC"', description: 'Timezone for cron evaluation' }),
    
    // Job template
    jobType: field.string({ description: 'Job type to create' }),
    jobVersion: field.int({ default: 1 }),
    payload: field.json({ isOptional: true, description: 'Default payload for created jobs' }),
    
    // Execution settings
    maxRetries: field.int({ default: 3 }),
    timeoutMs: field.int({ isOptional: true, description: 'Job timeout in milliseconds' }),
...
export const JobExecutionEntity = defineEntity({
  ...
  fields: {
    ...
    // Worker info
    workerId: field.string({ isOptional: true, description: 'ID of worker that processed' }),
    
    // Relations
    job: field.belongsTo('Job', ['jobId'], ['id'], { onDelete: 'Cascade' }),
  },
  indexes: [
    index.on(['jobId', 'attemptNumber']),
  ],
});
