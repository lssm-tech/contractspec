...
export function CodeBlock({
	...
	return (
		...
		>
			{filename && (
				<div className="flex items-center justify-between border-zinc-800 border-b bg-zinc-900/50 px-4 py-2 text-xs text-zinc-400">
					<span>{filename}</span>
				</div>
			)}
			<div className="relative">
				...
				{highlightedHtml ? (
					...
					<pre
						className={cn(
							'overflow-x-auto p-4 font-mono text-[13px] text-zinc-300 leading-6',
							showLineNumbers && 'line-numbers'
						)}
					>
						<code>{code}</code>
