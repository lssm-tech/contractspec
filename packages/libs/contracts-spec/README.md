...
Use these example packages when you want one focused, importable reference per contract layer. `agent`, `knowledge`, and `type` are covered through the exported agent surfaces, knowledge bindings/source configs, and schema models because this package does not expose a standalone `define*` helper for those three categories.
...
The package currently exposes **549 subpath exports**. This map is auto-generated for high-context navigation and AI grounding.
...
- `acp`: `capability(1)`, `feature(1)`, `command(7)`, `plain(6)`
- `agent`: `capability(1)`, `feature(1)`, `command(3)`, `event(4)`, `query(2)`, `form(1)`, `presentation(1)`, `dataView(1)`, `plain(11)`
- `app-config`: `capability(1)`, `feature(1)`, `contracts(1)`, `docblock(1)`, `plain(8)`
- `capabilities`: `docblock(1)`, `plain(7)`
- `context`: `capability(1)`, `feature(1)`, `command(1)`, `event(1)`, `query(2)`, `form(1)`, `presentation(1)`, `dataView(1)`, `plain(11)`
- `contract-registry`: `plain(3)`
- `control-plane`: `capability(5)`, `feature(1)`, `command(9)`, `event(10)`, `query(6)`, `plain(7)`
- `data-views`: `docblock(1)`, `plain(8)`
- `database`: `capability(1)`, `feature(1)`, `query(4)`, `dataView(1)`, `plain(7)`
- `docs`: `capability(1)`, `feature(1)`, `command(2)`, `event(2)`, `query(2)`, `form(1)`, `presentation(2)`, `dataView(3)`, `docs(2)`, `docblock(35)`, `plain(13)`
- `events`: `plain(1)`
- `examples`: `docblock(1)`, `plain(6)`
- `experiments`: `docblock(1)`, `plain(3)`
- `features`: `plain(6)`
- `forms`: `docblock(1)`, `plain(2)`
- `harness`: `capability(4)`, `feature(1)`, `command(3)`, `event(8)`, `query(5)`, `presentation(1)`, `dataView(3)`, `plain(11)`
- `install`: `plain(1)`
- `integrations`: `capability(4)`, `feature(4)`, `docblock(1)`, `plain(105)`
- `jobs`: `plain(12)`
- `jsonschema`: `plain(1)`
- `knowledge`: `capability(1)`, `feature(1)`, `docblock(1)`, `plain(21)`
- `llm`: `plain(4)`
- `markdown`: `plain(1)`
- `migrations`: `plain(1)`
...
- `operations`: `plain(5)`
- `ownership`: `plain(1)`
- `policy`: `docblock(1)`, `plain(8)`
- `presentations`: `docblock(1)`, `plain(4)`
- `product-intent`: `plain(16)`
- `prompt`: `plain(1)`
- `promptRegistry`: `plain(1)`
- `provider-ranking`: `capability(1)`, `feature(1)`, `command(3)`, `event(3)`, `query(3)`, `form(2)`, `presentation(1)`, `dataView(2)`, `plain(11)`
- `regenerator`: `docblock(1)`, `plain(7)`
- `registry`: `plain(1)`
- `registry-utils`: `plain(1)`
- `resources`: `plain(1)`
- `schema-to-markdown`: `plain(1)`
- `serialization`: `plain(3)`
- `telemetry`: `docblock(1)`, `plain(4)`
- `tests`: `plain(3)`
- `themes`: `plain(1)`
...
- `workflow`: `docblock(1)`, `plain(12)`
- `workspace-config`: `docblock(1)`, `plain(3)`
...
- DocBlocks (35):
  ...
  - `docs/tech/acp/acp-transport.docblock`
  - `docs/tech/agent/agent-execution.docblock`
  - `docs/tech/auth/better-auth-nextjs.docblock`
  - `docs/tech/cli.docblock`
  - `docs/tech/context/context-system.docblock`
  ...
  - `docs/tech/database/database-context.docblock`
  ...
  - `docs/tech/provider-ranking/provider-ranking-system.docblock`
...
- Plain exports (non-suffix artifacts): `105`
