...
export const chatbotArenaIngester: BenchmarkIngester = {
	...
	description:
		'Elo ratings from the LMSYS Chatbot Arena human preference leaderboard.',

	async ingest(options?: IngesterOptions): Promise<BenchmarkResult[]> {
		if (
			options?.dimensions?.length &&
			!options.dimensions.includes('reasoning')
		) {
			return [];
		}
		...
		let results = entries.map((entry): BenchmarkResult => {
			...
			const modelId =
				entry.key ?? entry.Model.toLowerCase().replace(/\s+/g, '-');
