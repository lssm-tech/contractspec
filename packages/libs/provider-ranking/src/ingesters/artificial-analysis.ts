...
export const artificialAnalysisIngester: BenchmarkIngester = {
	...
	async ingest(options?: IngesterOptions): Promise<BenchmarkResult[]> {
		...
		for (const entry of entries) {
			...
			if (
				(entry.tokens_per_second != null || entry.ttft_ms != null) &&
				(!dims || dims.has('latency'))
			) {
...
function computeLatencyScore(tokensPerSec?: number, ttftMs?: number): number {
	let score = 50;
...
function computeCostScore(inputCost?: number, outputCost?: number): number {
