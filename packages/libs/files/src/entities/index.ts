...
export const FileStatusEnum = defineEntityEnum({
  name: 'FileStatus',
  values: ['PENDING', 'UPLOADED', 'PROCESSING', 'READY', 'ERROR', 'DELETED'] as const,
  schema: 'lssm_files',
...
export const FileEntity = defineEntity({
  ...
  fields: {
    ...
    // Storage
    storageProvider: field.enum('StorageProvider', { default: 'LOCAL', description: 'Storage backend' }),
    storagePath: field.string({ description: 'Path in storage backend' }),
    storageKey: field.string({ isOptional: true, description: 'Storage key/bucket' }),
    
    // Integrity
    checksum: field.string({ isOptional: true, description: 'SHA-256 checksum' }),
    etag: field.string({ isOptional: true, description: 'Storage ETag' }),
    
    // Status
    status: field.enum('FileStatus', { default: 'PENDING', description: 'File status' }),
    
    // Access
    isPublic: field.boolean({ default: false, description: 'Whether file is publicly accessible' }),
    expiresAt: field.dateTime({ isOptional: true, description: 'Auto-delete timestamp' }),
    ...
    // Image-specific
    width: field.int({ isOptional: true, description: 'Image width in pixels' }),
    height: field.int({ isOptional: true, description: 'Image height in pixels' }),
...
export const FileVersionEntity = defineEntity({
  ...
  fields: {
    ...
    changes: field.json({ isOptional: true, description: 'Change description' }),
...
export const AttachmentEntity = defineEntity({
  ...
  fields: {
    ...
    // Attachment metadata
    attachmentType: field.string({ isOptional: true, description: 'Type of attachment (document, image, avatar, etc.)' }),
    name: field.string({ isOptional: true, description: 'Display name (overrides file name)' }),
    description: field.string({ isOptional: true, description: 'Attachment description' }),
    
    // Ordering
    order: field.int({ default: 0, description: 'Display order' }),
    
    // Metadata
    metadata: field.json({ isOptional: true, description: 'Attachment-specific metadata' }),
  ...
  indexes: [
    ...
    index.unique(['fileId', 'entityType', 'entityId'], { name: 'attachment_unique' }),
  ],
});
...
export const UploadSessionEntity = defineEntity({
  ...
  fields: {
    ...
    // Upload state
    uploadId: field.string({ isOptional: true, description: 'Storage upload ID' }),
    uploadedBytes: field.int({ default: 0, description: 'Bytes uploaded so far' }),
    uploadedParts: field.json({ isOptional: true, description: 'Completed part info' }),
    
    // Status
    status: field.string({ default: '"pending"', description: 'Session status' }),
    error: field.string({ isOptional: true, description: 'Error message if failed' }),
    
    // Result
    fileId: field.string({ isOptional: true, description: 'Resulting file ID' }),
