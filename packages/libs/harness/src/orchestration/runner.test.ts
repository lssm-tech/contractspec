...
function scriptedAdapter(
  ...
  handler: (input: HarnessStepExecutionInput) => Promise<HarnessStepExecutionResult>,
): HarnessExecutionAdapter {
  return {
    mode,
    supports: () => true,
...
describe("HarnessRunner", () => {
  ...
  it("evaluates scenarios and suites without state leakage across runs", async () => {
    ...
    const runner = new HarnessRunner({
      ...
      adapters: [
        scriptedAdapter("code-execution", async ({ step, target }) => ({
          ...
          output: { ok: true },
          artifacts: [{ kind: "step-summary", summary: "Loop and branch passed" }],
        })),
      ],
    });
    const scenario = defineHarnessScenario({
      ...
      assertions: [
        { key: "ok", type: "step-status", source: "validate-script", match: "completed" },
      ],
    });
    const suite = defineHarnessSuite({
      ...
      scenarios: [{ scenario: { key: scenario.meta.key, version: scenario.meta.version } }],
    });
    ...
    const suiteResult = await evaluationRunner.runSuiteEvaluation({ suiteKey: suite.meta.key });
