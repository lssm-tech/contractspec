...
export class HarnessEvaluationRunner {
  ...
  async runSuiteEvaluation(input: {
    ...
    const suite = this.options.suiteRegistry?.get(input.suiteKey, input.version);
    ...
    const passedScenarios = evaluations.filter((item) => item.status === "passed").length;
    const failedScenarios = evaluations.filter((item) => item.status === "failed").length;
    const blockedScenarios = evaluations.filter((item) => item.status === "blocked").length;
    ...
    return {
      ...
      summary: {
        ...
        blockedScenarios,
        evidenceCount,
        passRate: evaluations.length === 0 ? 0 : passedScenarios / evaluations.length,
        flakeRate: 0,
      },
    };
  }
