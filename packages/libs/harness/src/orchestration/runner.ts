...
import { classifyHarnessStep, canFallbackToVisual } from "../policy/classifier";
...
export class HarnessRunner {
  ...
  async runScenario(input: HarnessRunScenarioInput): Promise<HarnessRunScenarioResult> {
    ...
    const target = await this.options.targetResolver.resolve({
      ...
      allowlistedDomains:
        input.target?.allowlistedDomains ?? input.scenario.target.allowlistedDomains,
  ...
  private async executeWithAdapter(mode: string, input: Parameters<HarnessExecutionAdapter["execute"]>[0]) {
