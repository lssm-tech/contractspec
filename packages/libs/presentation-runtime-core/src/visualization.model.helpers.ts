...
export function createVisualizationMaps(
	...
	return {
		...
		measures: new Map(
			(spec.visualization.measures ?? []).map((measure) => [
				measure.key,
				measure,
			])
		),
...
function resolveAnnotation(
	...
	return {
		...
		y: annotation.yDataPath
			? toNumber(getAtPath(row, annotation.yDataPath))
			: undefined,
		start: annotation.startDataPath
			? getAtPath(row, annotation.startDataPath)
			: undefined,
		end: annotation.endDataPath
			? getAtPath(row, annotation.endDataPath)
			: undefined,
	};
}
