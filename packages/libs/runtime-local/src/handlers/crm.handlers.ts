...
// Note: We don't use LocalRow generics here - just cast the results
...
export function createCrmHandlers(db: LocalDatabase) {
  /**
  ...
  async function listDeals(input: ListDealsInput): Promise<ListDealsOutput> {
    ...
    // Get total value
    const valueResult = (await db.exec(
