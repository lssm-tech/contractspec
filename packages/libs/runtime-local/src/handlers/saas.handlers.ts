...
export function createSaasHandlers(db: LocalDatabase) {
  /**
   * List projects
   */
  ...
  async function deleteProject(id: string): Promise<void> {
    await db.run(`DELETE FROM saas_project WHERE id = ?`, [id]);
  }
  ...
  async function getSubscription(input: {
    ...
    const rows = (await db.exec(query, params)) as unknown as SubscriptionRow[];
