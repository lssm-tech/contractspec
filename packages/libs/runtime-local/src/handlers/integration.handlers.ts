...
export function createIntegrationHandlers(db: LocalDatabase) {
  /**
   * List integrations
   */
  ...
  async function connectService(input: ConnectServiceInput): Promise<Connection> {
    ...
    const rows = (await db.exec(
      `SELECT * FROM integration_connection WHERE id = ?`,
      [id]
    )) as unknown as ConnectionRow[];
  ...
  async function configureSync(input: ConfigureSyncInput): Promise<SyncConfig> {
    ...
    const rows = (await db.exec(
      `SELECT * FROM integration_sync_config WHERE id = ?`,
      [id]
    )) as unknown as SyncConfigRow[];
  ...
  async function mapFields(input: MapFieldsInput): Promise<FieldMapping[]> {
    ...
    for (const mapping of input.mappings) {
      ...
      const rows = (await db.exec(
        `SELECT * FROM integration_field_mapping WHERE id = ?`,
        [id]
      )) as unknown as FieldMappingRow[];
