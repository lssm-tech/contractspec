...
export function createAnalyticsHandlers(db: LocalDatabase) {
  /**
   * List dashboards
   */
  ...
  async function createDashboard(
    ...
    const rows = (await db.exec(
      `SELECT * FROM analytics_dashboard WHERE id = ?`,
      [id]
    )) as unknown as DashboardRow[];
  ...
  async function getDashboard(input: {
    ...
    const rows = (await db.exec(
      `SELECT * FROM analytics_dashboard ${whereClause}`,
      params
    )) as unknown as DashboardRow[];
  ...
  async function deleteWidget(widgetId: string): Promise<void> {
    await db.run(`DELETE FROM analytics_widget WHERE id = ?`, [widgetId]);
  }
