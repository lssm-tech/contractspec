...
export function createWorkflowHandlers(db: LocalDatabase) {
  /**
  ...
  async function listDefinitions(
    ...
    const rows = (await db.exec(
      ...
      [...params, limit, offset]
    )) as unknown as WorkflowDefinitionRow[];
  ...
  async function addStep(input: AddWorkflowStepInput): Promise<WorkflowStep> {
    ...
    const maxOrderResult = (await db.exec(
  ...
  async function approveStep(
    ...
    // Get next step
    const currentStep = (await db.exec(
      `SELECT * FROM workflow_step WHERE id = ?`,
      [instance.currentStepId]
    )) as unknown as WorkflowStepRow[];

    const nextSteps = (await db.exec(
  ...
  async function getApprovals(instanceId: string): Promise<WorkflowApproval[]> {
    const rows = (await db.exec(
      ...
      [instanceId]
    )) as unknown as WorkflowApprovalRow[];
