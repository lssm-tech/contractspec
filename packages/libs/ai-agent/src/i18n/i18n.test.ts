...
import { createAgentI18n, getDefaultI18n, resetI18nRegistry } from './messages';
import {
  resolveLocale,
  isSupportedLocale,
  DEFAULT_LOCALE,
...
describe('resolveLocale', () => {
  ...
  it('should use specLocale when provided', () => {
    expect(resolveLocale('fr')).toBe('fr');
    expect(resolveLocale('es')).toBe('es');
  });

  it('should prefer runtimeLocale over specLocale', () => {
...
describe('createAgentI18n', () => {
  ...
  it('should create an i18n instance with spec locale', () => {
    const i18n = createAgentI18n('fr');
    expect(i18n.locale).toBe('fr');
  });

  it('should prefer runtime locale over spec locale', () => {
    const i18n = createAgentI18n('fr', 'es');
...
describe('t() - English', () => {
  ...
  it('should return English strings by default', () => {
    const i18n = createAgentI18n('en');
    expect(i18n.t('knowledge.header')).toBe('# Reference Knowledge');
    expect(i18n.t('interop.md.toc')).toBe('## Table of Contents');
  });

  it('should interpolate placeholders', () => {
    const i18n = createAgentI18n('en');
    const result = i18n.t('error.missingToolHandler', { name: 'search' });
    expect(result).toBe('Missing handler for tool: search');
  });

  it('should interpolate multiple placeholders', () => {
    const i18n = createAgentI18n('en');
    const result = i18n.t('error.agentDuplicateTool', {
      key: 'my-agent',
      name: 'search',
    });
    expect(result).toBe('Agent my-agent has duplicate tool name: search');
  });

  it('should leave unmatched placeholders as-is', () => {
    const i18n = createAgentI18n('en');
    // Pass only one of two expected placeholders
    const result = i18n.t('error.agentDuplicateTool', { key: 'my-agent' });
...
describe('t() - French', () => {
  ...
  it('should return French strings', () => {
    const i18n = createAgentI18n('fr');
    expect(i18n.t('knowledge.header')).toBe('# Connaissances de référence');
    expect(i18n.t('interop.md.toc')).toBe('## Table des matières');
  });

  it('should interpolate placeholders in French', () => {
    const i18n = createAgentI18n('fr');
    const result = i18n.t('error.missingToolHandler', { name: 'search' });
    expect(result).toBe("Gestionnaire manquant pour l'outil : search");
  });
});
...
describe('t() - Spanish', () => {
  ...
  it('should return Spanish strings', () => {
    const i18n = createAgentI18n('es');
    expect(i18n.t('knowledge.header')).toBe('# Conocimiento de referencia');
    expect(i18n.t('interop.md.toc')).toBe('## Tabla de contenidos');
  });

  it('should interpolate placeholders in Spanish', () => {
    const i18n = createAgentI18n('es');
    const result = i18n.t('error.missingToolHandler', { name: 'search' });
    expect(result).toBe('Falta el manejador para la herramienta: search');
  });
});
