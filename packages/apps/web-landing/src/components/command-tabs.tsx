...
export function CommandTabs({
	...
	return (
		<div
			className={cn(
				'relative rounded-lg border bg-zinc-950 dark:border-zinc-800',
				className
			)}
			{...props}
		>
			<div className="flex items-center border-zinc-800 border-b bg-zinc-900/50 px-2 pt-2">
				{packageManagers.map((pm) => (
					<button
						key={pm}
						onClick={() => handleSelect(pm)}
						className={cn(
							'rounded-t-md border-transparent border-t border-r border-l px-4 py-2 font-medium text-xs text-zinc-400 hover:text-zinc-100',
							selected === pm && 'border-zinc-800 bg-zinc-950 text-zinc-100'
						)}
					>
						{pm}
					</button>
			...
			<div className="relative p-4">
				<CopyButton value={currentCommand} className="absolute top-4 right-4" />
				<pre className="overflow-x-auto font-mono text-[13px] text-white leading-6">
