...
export async function validateGeneratedRegistry(
	...
	if (!builtins) {
		errors.push('missing generated builtins.ts');
	} else {
		if (
			...
			errors.push(
				'generated builtins.ts must be catalog-only and cannot import example packages'
			);
		}
		for (const example of maintainedExamples) {
			const sourceDirectory = `packages/examples/${example.directory}`;
			if (!builtins.includes(`"packageName": "${example.packageName}"`)) {
				errors.push(
					`generated builtins.ts is missing catalog entry for ${example.packageName}`
				);
			}
			if (!builtins.includes(`"directory": "${sourceDirectory}"`)) {
				errors.push(
					`generated builtins.ts is missing source metadata for ${sourceDirectory}`
				);
			}
		}
	}
	...
	for (const dependencyName of Object.keys(dependencies)) {
		if (dependencyName.startsWith('@contractspec/example.')) {
			errors.push(
				`packages/modules/examples/package.json must not depend on ${dependencyName}`
			);
		}
	}
