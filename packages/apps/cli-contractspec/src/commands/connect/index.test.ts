...
describe("connect command", () => {
  it("exposes the implemented subcommand family", () => {
    ...
    expect(
      connectCommand.commands.find((command) => command.name() === "review")?.commands.map(
  ...
  it("keeps operator-facing flags for context, verify, and eval flows", () => {
    const context = connectCommand.commands.find((command) => command.name() === "context");
    const verify = connectCommand.commands.find((command) => command.name() === "verify");
    const hook = connectCommand.commands.find((command) => command.name() === "hook");
    const evaluate = connectCommand.commands.find((command) => command.name() === "eval");
    ...
    expect(
      hook?.commands.find((command) => command.name() === "contracts-spec")?.commands.map(
        (command) => command.name(),
      ),
    ).toEqual(["before-file-edit", "before-shell-execution", "after-file-edit"]);
    expect(evaluate?.description()).toContain("Run harness evaluation");
    expect(evaluate?.options.map((option) => option.long)).toContain("--registry");
    expect(evaluate?.options.map((option) => option.long)).toContain("--scenario");
  ...
  it("maps verdicts and configuration/runtime errors to the documented exit codes", () => {
    ...
    expect(connectErrorExitCode(new Error("No stored Connect decision."))).toBe(40);
