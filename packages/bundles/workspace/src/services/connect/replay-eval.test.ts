...
describe("connect replay and eval", () => {
  ...
  afterEach(() => {
    ...
    mockDetectImpact.mockResolvedValue(
		...
		const verified = await verifyConnectMutation(createAdapters(fs), {
			config: createConfig(),
			cwd: packageRoot,
			operation: "edit",
			packageRoot,
    ...
    const evaluation = await evaluateConnectDecision(
      ...
      },
      {
		...
		expect(
			(replay.trace as { decision: { id: string } }).decision.id,
		).toBe(runtimeDecisionId!);
		expect(
			capturedContext?.controlPlanePlanId,
			).toBe(
				(replay.trace as { decision: { actionPlan: { id: string } } }).decision.actionPlan.id,
			);
		expect(capturedContext?.controlPlaneExecutionId).toBe(
			runtimeDecisionId!,
		);
    ...
    expect(readFileSync(join(verified.historyDir, "evaluation-result.json"), "utf8")).toContain(
      '"ok": true',
    );
  });
});
