...
import { createVerifyService, VerifyService } from './verify-service';
	...
	() =>
		({
			...
			meta: {
				specName: 'test',
				specVersion: '1.0.0',
				implementationPath: 'code.ts',
				verifiedAt: 'now',
				duration: 0,
			},
			coverage: {
				scenarios: { covered: 0, total: 0 },
				errors: { handled: 0, total: 0 },
				fields: { implemented: 0, total: 0 },
			},
...
describe('VerifyService', () => {
	...
	const mockSpec = {
		key: 'test.op',
		version: '1.0.0',
	} as unknown as AnyOperationSpec;
