...
import { listTemplates, type TemplateDefinition } from '../../../templates/registry';
      ...
      <MarketingSection tone="default">
        <VStack gap="lg">
          <HStack gap="md" wrap="wrap">
            {filtered.map((t) => (
              <MarketingCard key={t.id} className="w-full md:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)]">
                <MarketingCardHeader>
                  <MarketingCardTitle>
                    {t.icon} {t.name}
                  </MarketingCardTitle>
                  <MarketingCardDescription>{t.description}</MarketingCardDescription>
                </MarketingCardHeader>
                <MarketingCardContent>
                  <VStack gap="md">
                    <HStack gap="sm" wrap="wrap">
                      {t.tags.slice(0, 6).map((tag) => (
                        <DsLink key={`${t.id}-${tag}`} href={`/templates?tag=${encodeURIComponent(tag)}`} variant="muted">
                          {tag}
                        </DsLink>
                      ))}
                    </HStack>
                    <HStack gap="sm" justify="between" wrap="wrap">
                      ...
                      <Button variant="outline" onPress={() => void 0} isDisabled>
