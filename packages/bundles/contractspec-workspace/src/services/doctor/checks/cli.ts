    ...
    return {
      ...
      fix: {
        ...
        apply: async (): Promise<FixResult> => {
          try {
            await execAsync('npm install -g @lssm/app.cli-contractspec', {
              cwd: ctx.workspaceRoot,
              timeout: 60000,
            });
...
async function checkCliInstallLocation(
  ...
  try {
    ...
    const hasDep =
      pkg.dependencies?.['@lssm/app.cli-contractspec'] !== undefined ||
      pkg.devDependencies?.['@lssm/app.cli-contractspec'] !== undefined;
    ...
    return {
      ...
      details: 'Consider adding @lssm/app.cli-contractspec to devDependencies',
      fix: {
        description: 'Add CLI as a dev dependency',
        apply: async (): Promise<FixResult> => {
          try {
            await execAsync('npm install -D @lssm/app.cli-contractspec', {
              cwd: ctx.workspaceRoot,
              timeout: 60000,
            });
