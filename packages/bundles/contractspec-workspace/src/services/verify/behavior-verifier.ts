...
function checkScenarioCoverage(
  ...
  const hasTestForScenario = codeLower.includes(scenarioNameLower) ||
...
function checkExampleCoverage(
  ...
  const inputCoverage = inputValues.length > 0 ? foundInputValues.length / inputValues.length : 1;
  const outputCoverage = outputValues.length > 0 ? foundOutputValues.length / outputValues.length : 1;
...
export function verifyBehavior(input: VerifyInput): VerificationReport {
  ...
  const issues: VerificationIssue[] = checks
    ...
    .map(c => ({
      ...
      category: c.type === 'scenario' ? 'scenario' as const
        : c.type === 'example' ? 'scenario' as const
