...
export const LogTypeEnum = defineEntityEnum({
  name: 'LogType',
  values: ['MESSAGE', 'TOOL_CALL', 'TOOL_RESULT', 'THINKING', 'ERROR', 'SYSTEM'] as const,
  schema: 'agent',
...
export const RunLogEntity = defineEntity({
  ...
  fields: {
    ...
    // Content
    role: field.string({ isOptional: true, description: 'Message role (user, assistant, system, tool)' }),
    ...
    // Relations
    run: field.belongsTo('AgentRun', ['runId'], ['id'], { onDelete: 'Cascade' }),
  },
  indexes: [
    index.on(['runId', 'sequence']),
    index.on(['runId', 'type']),
  ],
