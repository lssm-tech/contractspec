...
export const RunEntity = defineEntity({
  ...
  fields: {
    id: field.id(),
    organizationId: field.string({ description: 'Organization that owns this run' }),
    ...
    sessionId: field.string({ isOptional: true, description: 'Session ID for conversational context' }),
    parentRunId: field.string({ isOptional: true, description: 'Parent run ID for nested agent calls' }),
    // Input/Output
    input: field.json({ description: 'Input data for the run' }),
    output: field.json({ isOptional: true, description: 'Output data from the run' }),
    // Status
    status: field.enum('RunStatus', { default: 'QUEUED' }),
    errorMessage: field.string({ isOptional: true, description: 'Error message if run failed' }),
    errorCode: field.string({ isOptional: true, description: 'Error code if run failed' }),
    // Metrics
    ...
    totalIterations: field.int({ default: 0, description: 'Total iterations executed' }),
    durationMs: field.int({ isOptional: true, description: 'Total duration in milliseconds' }),
    // Cost tracking
    estimatedCostUsd: field.float({ isOptional: true, description: 'Estimated cost in USD' }),
    // Timestamps
    queuedAt: field.createdAt(),
    startedAt: field.dateTime({ isOptional: true, description: 'When execution started' }),
    completedAt: field.dateTime({ isOptional: true, description: 'When execution completed' }),
    // Metadata
    metadata: field.json({ isOptional: true, description: 'Additional run metadata' }),
    // Relations
...
export const RunStepEntity = defineEntity({
  ...
  fields: {
    ...
    // Step details
    toolId: field.string({ isOptional: true, description: 'Tool ID if tool call' }),
    toolName: field.string({ isOptional: true, description: 'Tool name for display' }),
    ...
    errorMessage: field.string({ isOptional: true, description: 'Error message if step failed' }),
    // Metrics
    tokensUsed: field.int({ default: 0, description: 'Tokens used in this step' }),
    durationMs: field.int({ isOptional: true, description: 'Step duration in milliseconds' }),
    // Timestamps
    startedAt: field.createdAt(),
    completedAt: field.dateTime({ isOptional: true, description: 'When step completed' }),
    // Relations
...
export const RunLogEntity = defineEntity({
  ...
  fields: {
    id: field.id(),
    runId: field.foreignKey({ description: 'Run ID' }),
    stepId: field.string({ isOptional: true, description: 'Step ID if related to a specific step' }),
    ...
    // Context
    source: field.string({ isOptional: true, description: "Log source: 'agent', 'tool', 'system'" }),
    traceId: field.string({ isOptional: true, description: 'Trace ID for distributed tracing' }),
    spanId: field.string({ isOptional: true, description: 'Span ID for distributed tracing' }),
    // Timestamp
