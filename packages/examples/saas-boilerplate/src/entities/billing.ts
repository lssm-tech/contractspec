...
export const BillingUsageEntity = defineEntity({
  ...
  fields: {
    ...
    unit: field.string({ isOptional: true, description: 'Unit of measurement' }),
    ...
    // Source
    sourceId: field.string({ isOptional: true, description: 'Source of usage (e.g., request ID)' }),
...
export const UsageLimitEntity = defineEntity({
  ...
  fields: {
    ...
    // Scope
    planId: field.string({ isOptional: true, description: 'Plan this limit applies to' }),
    organizationId: field.string({ isOptional: true, description: 'Org-specific override' }),
    
    // Limit
    feature: field.string({ description: 'Feature being limited' }),
    limit: field.int({ description: 'Maximum allowed usage' }),
    resetPeriod: field.string({ default: '"monthly"', description: 'When limit resets' }),
    
    // Soft/hard limit
    isSoftLimit: field.boolean({ default: false, description: 'Whether to warn vs block' }),
    overage: field.boolean({ default: false, description: 'Whether overage is allowed' }),
    overageRate: field.float({ isOptional: true, description: 'Cost per unit over limit' }),
