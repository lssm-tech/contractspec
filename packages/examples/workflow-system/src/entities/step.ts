...
export const WorkflowStepEntity = defineEntity({
  ...
  fields: {
    ...
    approvalMode: field.enum('ApprovalMode', { default: 'ANY', isOptional: true }),
    approverRoles: field.string({ isArray: true, description: 'Roles that can approve' }),
    approverUserIds: field.string({ isArray: true, description: 'Specific users that can approve' }),
    escalationConfig: field.json({ isOptional: true, description: 'Escalation rules' }),
    
    // Task configuration (when type is TASK)
    assigneeRoles: field.string({ isArray: true, description: 'Roles that can be assigned' }),
    taskTemplate: field.json({ isOptional: true, description: 'Template for task creation' }),
    
    // Condition configuration (when type is CONDITION)
    conditionExpression: field.string({ isOptional: true, description: 'Expression for branching' }),
    
    // Wait configuration (when type is WAIT)
    waitDuration: field.int({ isOptional: true, description: 'Wait duration in seconds' }),
    waitForEvent: field.string({ isOptional: true, description: 'Event name to wait for' }),
    
    // Action configuration (when type is ACTION)
    actionType: field.string({ isOptional: true, description: 'Action to execute' }),
    actionConfig: field.json({ isOptional: true, description: 'Action parameters' }),
    
    // Timeout & SLA
    timeoutSeconds: field.int({ isOptional: true, description: 'Step timeout' }),
    slaSeconds: field.int({ isOptional: true, description: 'SLA deadline' }),
    
    // Notifications
    notifyOnEnter: field.json({ isOptional: true, description: 'Notification config when entering step' }),
    notifyOnExit: field.json({ isOptional: true, description: 'Notification config when exiting step' }),
  ...
  indexes: [
    index.on(['workflowDefinitionId', 'key']).unique(),
