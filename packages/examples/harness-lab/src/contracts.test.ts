...
describe("@contractspec/example.harness-lab contracts", () => {
	test("exports stable scenario, suite, and feature metadata", () => {
		...
		expect(HarnessLabDualModeSuite.meta.key).toBe("harness-lab.dual-mode.suite");
		expect(HarnessLabDualModeSuite.scenarios.map((entry) => entry.scenario.key)).toEqual([
			HarnessLabSandboxScenario.meta.key,
			HarnessLabBrowserScenario.meta.key,
		]);
		expect(HarnessLabDualModeSuite.scenarios.every((entry) => entry.required)).toBe(
			true,
		);
